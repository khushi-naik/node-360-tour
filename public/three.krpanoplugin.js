...
function krpanoplugin()
{
	...
	function start()
	{
		...
		krpano.set("events[__threejs__].keep", false); //to not have models on all scenes
	...
	function render_frame()
	{
		...
		// do scene updates
		//update_scene(); //to make the cube rotate
	...
	function update_object_properties(obj)
	{
		...
		//obj.rotation._x = p.rx*M_RAD;
		//obj.rotation._y = p.ry*M_RAD;
		//obj.rotation._z = p.rz*M_RAD;
		//obj.rotation._order = p.rorder;
	...
	const colors = [0xff3333, 0xfff933, 0x338aff, 0x6e33ff, 0xff9933, 0x0aab47];
	...
	function build_scene()
	{
		...
		var move_box_up = document.getElementById("move-up");
		var move_box_left = document.getElementById("move-left");
		var move_box_right = document.getElementById("move-right");
		var move_box_down = document.getElementById("move-down");
		console.log("no of up" +move_box_up);
		console.log("no of left" +move_box_left);
		console.log("no of right" +move_box_right);
		console.log("no of down" +move_box_down);
		...
		//box = new THREE.Mesh(new THREE.BoxGeometry(500,500,500), material1);
		box = new THREE.Mesh(new THREE.BoxGeometry(500,500,500), new THREE.MeshBasicMaterial({ map: THREE.ImageUtils.loadTexture('/rubik.jpg')}));
		assign_object_properties(box, "box", {ath:40, atv:25, depth:2000});
		scene.add( box );

		move_box_left.addEventListener("click", function(){
			var curr= box.properties.ath;
			curr=curr-5;
			console.log("current v h", curr, box.properties.atv);
			socket.emit('position', curr, box.properties.atv);
			
		});

		move_box_right.addEventListener("click", function(){
			var curr= box.properties.ath;
			curr=curr+5;
			console.log("current v h", curr, box.properties.atv);
			socket.emit('position', curr, box.properties.atv);
		...
		move_box_up.addEventListener("click", function(){
			var curr= box.properties.atv;
			curr=curr-5;
			console.log("current v h", box.properties.ath, curr);
			socket.emit('position', box.properties.ath, curr);
			
		});

		move_box_down.addEventListener("click", function(){
			var curr= box.properties.atv;
			curr=curr+5;
			console.log("current v h", box.properties.ath, curr);
			socket.emit('position', box.properties.ath, curr);
		...
		socket.on('position', function(ath, atv){
			console.log("updated v h", ath, atv);
			assign_object_properties(box, "box", {ath:ath, atv:atv, depth:2000});
		});
